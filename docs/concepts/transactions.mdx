...
The account setting `max_captures_per_authorization` limits how many captures one authorization allows. The default is `5`. See [Authorize and Capture Payments](/guides/payments/authorize-and-capture-payments) for the limit and the error returned when you pass it.
