...
<Alert type="warning">3DS is gated per account and is off by default. Contact PublicSquare to enable 3DS on your account before integrating.</Alert>
...
|------------------------------|------------------------------------------------------------|---------------------------------------------------|
| Session created              | In the browser, via the Elements SDK                       | Server-side, by PublicSquare at `confirm`         |
...
| `session_id`               | Iframe only. The PublicSquare 3DS session id (`id`) returned by the Elements SDK `createSession`.                 |
...
Created in the browser by the Elements SDK `createSession` (which also registers it with PublicSquare) and retrievable via [Get 3DS Session](/api/financial/three-ds-get-by-id).

| Field                    | Description                                                                                    |
|--------------------------|------------------------------------------------------------------------------------------------|
| `id`                     | The **PublicSquare** session id. Pass this to `confirm` and `complete`.                        |
...
<Alert>Two ids are in play: the PublicSquare `id` drives the `confirm`/`complete` API calls, and `bt_session_id` drives the in-page challenge.</Alert>
...
- [Get 3DS Session](/api/financial/three-ds-get-by-id) — API reference
