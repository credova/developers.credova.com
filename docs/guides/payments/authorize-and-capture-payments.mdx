...
<Alert type="warning">Not all payment processors support multiple partial captures. If your account's processor does not support it, a second capture on the same authorization is rejected.</Alert>
...
<Alert type="warning">`is_final` defaults to `true`. Pass `is_final: false` explicitly on every interim capture, or the authorization closes out early.</Alert>
...
- You omit `is_final`, or pass `is_final: true`. `is_final` defaults to `true`.
- You send `amount: 0`. This always releases the remaining authorization, regardless of `is_final`.
...
A payment reports four amounts. They all use the smallest currency unit, so `1000` is $10.00 USD.

| Field | Meaning |
|-------|---------|
| `amount` | The amount you requested when you created the payment. It does not change |
| `amount_capturable` | The authorized amount you can still capture |
| `amount_charged` | The amount captured so far. This is the amount the customer pays |
| `amount_refunded` | The amount refunded so far |

After an authorization, `amount_charged` is `0` and `amount_capturable` holds the full amount. After a capture, the captured value moves to `amount_charged` and `amount_capturable` drops by the same value.

A partial capture leaves the rest in `amount_capturable`, so a second capture can take it.

To know what the customer paid, read `amount_charged`. Do not read `amount`, which only records what you asked for.
