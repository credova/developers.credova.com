...
If a payment has [multiple partial captures](/guides/payments/authorize-and-capture-payments#multiple-partial-captures), pass `payment_capture_id` to cancel one capture instead of the whole payment.
...
```shell showLineNumbers title="Void a Pending Capture"
curl 'https://api.publicsquare.com/payments/cancel' \
-X 'POST' \
-H 'X-API-KEY: <SECRET_API_KEY>' \
-H 'Content-Type: application/json' \
...
```shell showLineNumbers title="Release the Remaining Authorization"
curl 'https://api.publicsquare.com/payments/cancel' \
-X 'POST' \
-H 'X-API-KEY: <SECRET_API_KEY>' \
-H 'Content-Type: application/json' \
...
<Alert type="warning">A release is rejected with a `422` while any capture is still pending: `payment has a capture still processing; wait for it to settle before releasing the remainder.`</Alert>

Voiding a capture that has already settled is rejected. Use a [refund](/guides/refunds/refund-payments#refunding-a-specific-capture) instead:

```json showLineNumbers title="Capture Already Settled"
{
  "title": "Error",
  "status": 400,
