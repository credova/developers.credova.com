...
In this guide, we will set up Credova Elements SDKs to capture instantly verified bank accounts in a frontend application, securely authorize them, and store the bank account data while avoiding PCI requirements.
...
Credova has partnered with [Flinks](https://www.flinks.com/) to provide a secure way to collect bank accounts.
...
```
<Alert>If Credova has not yet received a response back from Flinks, we will return `bank account verification result is still in progress.` validation message.</Alert>
...
<Alert type="warning">Bank accounts with MFA enabled do not support nightly balance refresh.</Alert>
